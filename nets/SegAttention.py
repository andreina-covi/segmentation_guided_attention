...
class SegAttention(nn.Module):
    def __init__(
        ...
        if restore is not None:
            self.seg_net.load_state_dict(torch.load(restore, map_location=device))
        ...
        self.seg_dims = self.seg_net(sample)[
            0
        ].size()  # for layer size definitionlayers
    ...
    def single_forward(self, batch):
        ...
        seg_output = (
            self.softmax(self.seg_net(batch)[0])
            if self.softmax is not None
            else self.seg_net(batch)[0]
        )
        seg_output = self.semantic_drop(seg_output)
        seg_output_permuted = seg_output.permute([2, 3, 0, 1])
        segmentation = seg_output_permuted.contiguous().view(
            self.seg_dims[2] * self.seg_dims[3], batch_size, NUM_CLASSES
        )
        ...
        x = (
            x.permute([1, 0, 2])
            .contiguous()
            .view(batch_size, self.hidden_dim * self.seg_dims[2] * self.seg_dims[3])
        )
        x = self.dropout(self.activation(self.fc(x)))
        x = self.output(x)

        return {"output": x, "segmentation": seg_output, "attention": attn_list}
...
if __name__ == "__main__":
    ...
    dist.init_process_group(
        "gloo", init_method="file:///tmp/tmpfile", rank=0, world_size=1
    )
    INPUT_SIZE = "340,480"
    h, w = map(int, INPUT_SIZE.split(","))
    model = SegAttention(
        models.resnet50, image_size=(h, w), restore=RESTORE_FROM, n_heads=1, n_layers=1
    )
